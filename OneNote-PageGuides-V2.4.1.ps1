...
<#
.SYNOPSIS
    OneNote Page Guides 2.4.1 - removable visual guides, not native print boundaries.
.DESCRIPTION
...
.EXAMPLE
    .\OneNote-PageGuides-V2.4.1.ps1 -Action SelfTest
.EXAMPLE
    .\OneNote-PageGuides-V2.4.1.ps1 -Action Install -CreateDesktopShortcuts
.EXAMPLE
    .\OneNote-PageGuides-V2.4.1.ps1 -Action Refresh -Pages 2 -WhatIf
.EXAMPLE
    .\OneNote-PageGuides-V2.4.1.ps1 -Action Add -Pages 2
.EXAMPLE
    .\OneNote-PageGuides-V2.4.1.ps1 -Action Refresh -GuideMode PrintArea -PagePitchPoints 720
.EXAMPLE
    .\OneNote-PageGuides-V2.4.1.ps1 -Action Recover -RecoveryFile 'C:\path\transaction.json'
.NOTES
    Version: 2.4.1
    Patch: permit the exact OneNote xmlns declarations on recovery images while
    continuing to reject object IDs and other non-insertable image attributes.
    Includes recovery namespace and object-ID regression tests.
...
param(
    [ValidateSet('Status','Add','Refresh','Remove','Recover','Install','Uninstall','SelfTest')]
...
function Select-TargetPage {
    ...
    if (-not $UseConsole -and $null -ne (Get-Command Out-GridView -ErrorAction SilentlyContinue)) {
        ...
        $picked = @()
        try {
            $picked = @($ordered | Out-GridView -Title 'OneNote Page Guides 2.4.1 - choose ONE target page' -OutputMode Single)
        }
        catch {
            $gridFailed = $true
...
function New-GuidePayload {
    ...
    for ($p=1; $p -le $Count; $p++) {
        ...
        $marker = 'v2;version=2.4.1;batch={0};page={1};mode={2};pitch={3};' -f $Batch,$p,$Geometry.Mode,(Format-Point $Geometry.Pitch)
...
function Install-Utility {
    if ([string]::IsNullOrWhiteSpace($script:RunningFile) -or -not (Test-Path -LiteralPath $script:RunningFile -PathType Leaf)) {
        throw 'Run this from the saved V2.4 .ps1 file.'
    }
    ...
    $shortcuts = @(
        @('Refresh OneNote Page Guides.lnk',$refresh,'Choose a page and refresh V2.4 visual guides'),
    ...
    Write-Host 'OneNote Page Guides 2.4.1 installed. No OneNote content was changed.'
...
function Uninstall-Utility {
    $names = @('Refresh OneNote Page Guides.lnk','Remove OneNote Page Guides.lnk',
        'OneNote Page Guides Status.lnk','OneNote Page Guides Self-Test.lnk')
...
try {
    ...
    if ($Action -eq 'Install') {
        ...
        if ($PSCmdlet.ShouldProcess($script:InstallDirectory,'Install V2.4.1 and replace utility shortcuts')) { Install-Utility }
        return
    }
